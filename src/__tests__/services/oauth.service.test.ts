...
describe("oauth.service", () => {
  ...
  test("getAuthorizationUrl constructs google url with extra params", () => {
    jest.doMock("@config/env", () => ({
      env: {
        GOOGLE_CLIENT_ID: "gclient",
        GOOGLE_CLIENT_SECRET: "gsecret",
        GOOGLE_CALLBACK_URL: "https://cb",
        GITHUB_CLIENT_ID: "ghclient",
        GITHUB_CLIENT_SECRET: "ghsecret",
        GITHUB_CALLBACK_URL: "https://ghcb",
      },
    }));
  ...
  test("getAuthorizationUrl throws when provider not configured", () => {
    jest.doMock("@config/env", () => ({ env: { GOOGLE_CLIENT_ID: "" } }));
    const svc = require("@services/oauth.service");
    expect(() => svc.getAuthorizationUrl("google", "s")).toThrow(AUTH_MESSAGES.OAUTH_PROVIDER_NOT_CONFIGURED);
  });

  test("exchangeCodeForProfile - google success and token failure paths", async () => {
    jest.doMock("@config/env", () => ({
      env: {
        GOOGLE_CLIENT_ID: "gclient",
        GOOGLE_CLIENT_SECRET: "gsecret",
    ...
    // token exchange failure (non-ok)
    global.fetch = jest.fn().mockResolvedValueOnce({ ok: false });
    await expect(svc.exchangeCodeForProfile("google", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_TOKEN_EXCHANGE_FAILED);

    // successful token exchange + missing access_token
    ...
    global.fetch = jest
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ email: "a@b.com", name: "A", id: "id1" }) });

    const profile = await svc.exchangeCodeForProfile("google", "code");
    expect(profile.email).toBe("a@b.com");
  });

  test("exchangeCodeForProfile - github email discovery flow", async () => {
    jest.doMock("@config/env", () => ({
      env: {
        GITHUB_CLIENT_ID: "ghclient",
        GITHUB_CLIENT_SECRET: "ghsecret",
    ...
    global.fetch = jest
      ...
      // token exchange
      .mockResolvedValueOnce({ ok: true, json: async () => ({ access_token: "tok" }) })
      // user endpoint returns null email
      .mockResolvedValueOnce({ ok: true, json: async () => ({ email: null, name: "N", login: "l", id: 123 }) })
      // emails endpoint returns list
      .mockResolvedValueOnce({ ok: true, json: async () => [{ email: "primary@x.com", primary: true, verified: true }] });

    const profile = await svc.exchangeCodeForProfile("github", "code");
...
describe("oauth.service", () => {
  ...
  test("exchangeCodeForProfile throws when token endpoint returns non-ok", async () => {
    ...
    await expect(exchangeCodeForProfile("google", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_TOKEN_EXCHANGE_FAILED);
  });
  ...
  test("exchangeCodeForProfile throws when google userinfo endpoint fails", async () => {
    fetchSpy = jest.spyOn(global as any, "fetch").mockImplementationOnce(async () => ({ ok: true, json: async () => ({ token_type: "bearer" }) }));
  ...
  test("exchangeCodeForProfile throws when google userinfo endpoint fails2", async () => {
    ...
    await expect(exchangeCodeForProfile("google", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_USER_INFO_FAILED);
  });
  ...
  test("exchangeCodeForProfile throws when google userinfo missing email", async () => {
    ...
    await expect(exchangeCodeForProfile("google", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_EMAIL_MISSING);
  });
  ...
  test("exchangeCodeForProfile throws when github user endpoint fails", async () => {
    ...
    await expect(exchangeCodeForProfile("github", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_USER_INFO_FAILED);
  });
  ...
  test("exchangeCodeForProfile throws when github emails endpoint not ok and no email found", async () => {
    ...
    await expect(exchangeCodeForProfile("github", "code")).rejects.toThrow(AUTH_MESSAGES.OAUTH_EMAIL_MISSING);
  });
});
